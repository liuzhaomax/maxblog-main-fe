...
pipeline {
    ...
    environment {
        ...
        harborRepo = "maxblog-main-fe"
        Container_port = "9601" // 启用随机端口，会被赋值
        Host_port = "9601" // 启用随机端口，会被赋值
        JobName = "maxblog-main-fe/main"
        ...
        stage("Checkout") {
            steps {
                ...
                timeout(time: 3, unit: "MINUTES"){
                    checkout([$class: "GitSCM", branches: [[name: '**']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/liuzhaomax/maxblog-main-fe.git']]])
                }
                echo "--------------------- Checkout End ---------------------"
            }
        }
